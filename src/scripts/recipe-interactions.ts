...
class RecipeInteractionManager {
  ...
  private addLocalInfoToggle(): void {
    const ohjeHeader = document.querySelector('#ohje') as HTMLElement;
    ...
    const localToggle = toggleContainer.querySelector('.local-info-toggle') as HTMLInputElement;
    if (localToggle) {
      localToggle.checked = isInfoModeEnabled;
      
      localToggle.addEventListener('change', (e: Event) => {
    ...
    instructionLists.forEach((list, listIndex) => {
      ...
      steps.forEach((step, stepIndex) => {
        ...
        step.addEventListener('click', (e: MouseEvent) => {
          ...
          if (target.closest('.recipe-info-button')) {
  ...
  private handleInfoButtonClick(event: Event): void {
    const target = event.target as HTMLElement;
    const infoButton = target.closest('.recipe-info-button') as HTMLElement;
    
    if (!infoButton) return;
    
    const step = infoButton.closest('.recipe-instruction-step') as HTMLElement;
    if (!step) return;
    
    // Toggle the step info visibility
    const isEnabled = step.classList.contains('step-info-enabled');
    
    if (isEnabled) {
      step.classList.remove('step-info-enabled');
      infoButton.setAttribute('aria-label', 'Näytä lisätiedot');
      infoButton.setAttribute('title', 'Näytä lisätiedot');
    } else {
      step.classList.add('step-info-enabled');
